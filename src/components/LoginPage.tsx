...
import { signIn, resendVerificationEmail } from '../utils/firebaseUtils';
    ...
    try {
      ...
      if (result.error) {
        if ('alreadyVerified' in result && result.alreadyVerified) {
          setVerificationMessage('Your email is already verified! You can sign in now.');
          setNeedsVerification(false);
        } else {
          setError(result.error.message);
        }
            ...
            {verificationMessage && (
              <div className="p-4 rounded-lg bg-green-50 border border-green-200">
                <div className="flex items-center">
                  ...
                  <span className="text-green-700 text-sm">{verificationMessage}</span>
                </div>
              </div>
            )}
