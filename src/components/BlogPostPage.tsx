...
import { Calendar, User, ArrowLeft, ArrowRight, BookOpen, PenTool, GraduationCap, Clock } from 'lucide-react';
...
LinkedIn research shows that professionals with error-free profiles receive 14% more profile views. In job applications, 58% of hiring managers say they immediately discard resumes with grammar errors.
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center mb-4">
            <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">
              {post.category === 'Study Tips' && <BookOpen className="w-4 h-4 mr-1" />}
              {post.category === 'Writing Skills' && <PenTool className="w-4 h-4 mr-1" />}
              {post.category === 'Academic Skills' && <GraduationCap className="w-4 h-4 mr-1" />}
              {!['Study Tips', 'Writing Skills', 'Academic Skills'].includes(post.category) && <BookOpen className="w-4 h-4 mr-1" />}
              {post.category}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between text-gray-500 dark:text-gray-400 mb-6">
        ...
        <div className="mt-8 pt-8 border-t border-gray-200 dark:border-gray-700">
          <div className="flex flex-wrap gap-2">
            {post.tags.map((tag) => (
              <span
                key={tag}
                className="inline-block px-3 py-1 text-sm bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-full"
              >
                #{tag}
              </span>
            ))}
        ...
        <div className="mt-12 pt-8 border-t border-gray-200 dark:border-gray-700">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {nextPost && (
              <Link
                to={`/blog/${nextPost.id}`}
                className="group p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-lg transition-shadow md:text-right"
              >
                <div className="flex items-center justify-end text-blue-600 dark:text-blue-400 mb-2">
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 py-16">
        <div className="max-w-4xl mx-auto text-center px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-xl text-gray-600 dark:text-gray-300 mb-8">
            Join StudyWrite and start improving your writing with AI-powered assistance.
          </p>
          <Link
            to="/signup"
